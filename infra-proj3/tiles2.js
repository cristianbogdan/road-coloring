...
//    map.getPane('years').style.pointerEvents = 'none';
    ...
    var osmUrl = 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
        ...
        roadQUrl1= MAP_ROOT+'/infraVector/{z}/{x}/{y}.json',
...
//      roadQMap1= L.tileLayer(roadQUrl1),
    ...
    var geojsonTileLayer = new L.TileLayer.GeoJSON(roadQUrl1, {
        //clipTiles: true,
...
*/
    ...
    var yearsLayer= L.tileLayer(MAP_ROOT+'/infra/{z}/{x}/{y}.png', {pane: "years", autoZIndex:false});
    ...
    L.control.layers(
        {
            ...
            {
                "Projects":geojsonTileLayer,
                "Years": yearsLayer,
            }       
    ).addTo(map);

    function hackYears(){
        // show years on top of the segments
        // by moving the yearsLayer DIV as a child in the overlay DIV
        // Leaflet will continue to update it, not knowing that it was moved...
        map._panes.overlayPane.append(yearsLayer._container);
    }
    map.on('overlayadd', hackYears);
    changeUrl();
    hackYears();
}
